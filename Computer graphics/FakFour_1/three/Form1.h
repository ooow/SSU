...
#pragma endregion
	...
	private: System::Void Form1_Paint(System::Object^  sender, System::Windows::Forms::PaintEventArgs^  e) {
				 ...
				 /// Рисуем область, относительно которой будет отсекаться рисунок
				 for (int i = 0; i < polygons.Count; i++) 
				 {
					 ...
					 P1 = PClip(P1, Pmin, Pmax); // используем отсечение многоугольников из задания 7 
					 if (P1->Count != 0) {
						 a = P1[P1->Count - 1];
						 for (int j = 0; j < P1->Count; j++) {
							 b = P1[j];
							 g->DrawLine(redPen, a.x, a.y, b.x, b.y);
							 a = b;
						 }
					 }
				 }
				 for (int i = 0; i < lines.Count; i++) {
					 // Каждую точку конца отрезка превратим в тройку однородных координат
					 vec A, B;	 
					 point2vec(lines[i].start, A);
					 point2vec(lines[i].end, B);
					 vec A1, B1;
					 // применим к ним преобразование T
					 // получим точки в однородных координатах A1 и B1
					 timesMatVec(T,A,A1); 
					 timesMatVec(T,B,B1);
					 // переведем полученные координаты обратно в декартову систему
					 point a, b;
					 vec2point(A1, a);
					 vec2point(B1, b);
					 // изобразим отрезок в полученных координатах
					 if (newclip(a, b, P1)) // используем алгоритм отсечение отрезков рисунка относительно многоугольной области
					 {
						 g->DrawLine(blackPen, a.x, a.y, b.x, b.y);
						 float namex = (a.x + b.x) / 2, namey = (a.y + b.y) / 2;
						 if (drawNames) {
